...
public class ItemStack implements Cloneable, ConfigurationSerializable, Translatable {
    ...
    public void setType(@NotNull Material type) {
        ...
        this.type = type;
        if (this.meta != null) {
            this.meta = Vulcanium.getItemFactory().asMetaFor(meta, type);
        }
        if (type.isLegacy()) {
            createData((byte) 0);
        } else {
    ...
    public MaterialData getData() {
        Material mat = Vulcanium.getUnsafe().toLegacy(getType());
    ...
    public boolean isSimilar(@Nullable ItemStack stack) {
        ...
        Material comparisonType = (this.type.isLegacy()) ? Vulcanium.getUnsafe().fromLegacy(this.getData(), true) : this.type; // This may be called from legacy item stacks, try to get the right material
        return comparisonType == stack.getType() && getDurability() == stack.getDurability() && hasItemMeta() == stack.hasItemMeta() && (hasItemMeta() ? Vulcanium.getItemFactory().equals(getItemMeta(), stack.getItemMeta()) : true);
    }
    ...
    public void addUnsafeEnchantment(@NotNull Enchantment ench, int level) {
        ItemMeta itemMeta = (meta == null ? meta = Vulcanium.getItemFactory().getItemMeta(type) : meta);
        if (itemMeta != null) {
    ...
    public Map<String, Object> serialize() {
        ...
        result.put("v", Vulcanium.getUnsafe().getDataVersion()); // Include version to indicate we are using modern material names (or LEGACY prefix)
        ...
        ItemMeta meta = getItemMeta();
        if (!Vulcanium.getItemFactory().equals(meta, null)) {
    ...
    public static ItemStack deserialize(@NotNull Map<String, Object> args) {
        ...
        if (version < 0) {
            ...
            type = Vulcanium.getUnsafe().fromLegacy(new MaterialData(type, dataVal), true);
            ...
            type = Vulcanium.getUnsafe().getMaterial((String) args.get("type"), version);
        }
    ...
    public ItemMeta getItemMeta() {
        return this.meta == null ? Vulcanium.getItemFactory().getItemMeta(this.type) : this.meta.clone();
    }
    ...
    public boolean hasItemMeta() {
        return !Vulcanium.getItemFactory().equals(meta, null);
    }
    ...
    private boolean setItemMeta0(@Nullable ItemMeta itemMeta, @NotNull Material material) {
        ...
        if (!Vulcanium.getItemFactory().isApplicable(itemMeta, material)) {
            return false;
        }
        this.meta = Vulcanium.getItemFactory().asMetaFor(itemMeta, material);

        Material newType = Vulcanium.getItemFactory().updateMaterial(meta, material);
    ...
    public String getTranslationKey() {
        return Vulcanium.getUnsafe().getTranslationKey(this);
    }
    ...
    public boolean isEmpty() {
        return this.type.isAir() || this.amount <= 0;
    }
}