...
import dev.vulcanium.entity.*;
import dev.vulcanium.event.entity.CreatureSpawnEvent;
import dev.vulcanium.generator.BiomeProvider;
import dev.vulcanium.generator.BlockPopulator;
import dev.vulcanium.generator.ChunkGenerator;
import dev.vulcanium.generator.WorldInfo;
...
import dev.vulcanium.metadata.Metadatable;
...
import dev.vulcanium.plugin.messaging.PluginMessageRecipient;
import dev.vulcanium.util.Vector;
import dev.vulcanium.util.*;
import dev.vulcanium.world.chunks.Chunk;
import dev.vulcanium.world.chunks.ChunkSnapshot;
...
public interface World extends RegionAccessor, WorldInfo, PluginMessageRecipient, Metadatable, PersistentDataHolder, Keyed {
    ...
    default @NotNull Collection<Player> getNearbyPlayers(@NotNull Location loc, double radius) {
        return this.getNearbyEntitiesByType(Player.class, loc, radius, radius, radius);
    }

    default @NotNull Collection<Player> getNearbyPlayers(@NotNull Location loc, double xzRadius, double yRadius) {
        return this.getNearbyEntitiesByType(Player.class, loc, xzRadius, yRadius, xzRadius);
    }

    default @NotNull Collection<Player> getNearbyPlayers(@NotNull Location loc, double xRadius, double yRadius, double zRadius) {
        return this.getNearbyEntitiesByType(Player.class, loc, xRadius, yRadius, zRadius);
    }

    default @NotNull Collection<Player> getNearbyPlayers(@NotNull Location loc, double radius, @Nullable Predicate<? super Player> predicate) {
        return this.getNearbyEntitiesByType(Player.class, loc, radius, radius, radius, predicate);
    }

    default @NotNull Collection<Player> getNearbyPlayers(@NotNull Location loc, double xzRadius, double yRadius, @Nullable Predicate<? super Player> predicate) {
        return this.getNearbyEntitiesByType(Player.class, loc, xzRadius, yRadius, xzRadius, predicate);
    }

    default @NotNull Collection<Player> getNearbyPlayers(@NotNull Location loc, double xRadius, double yRadius, double zRadius, @Nullable Predicate<? super Player> predicate) {
        return this.getNearbyEntitiesByType(Player.class, loc, xRadius, yRadius, zRadius, predicate);
    }
    default <T extends Entity> @NotNull Collection<T> getNearbyEntitiesByType(@Nullable Class<? extends T> clazz, @NotNull Location loc, double radius) {
        return this.getNearbyEntitiesByType(clazz, loc, radius, radius, radius, (Predicate)null);
    }

    default <T extends Entity> @NotNull Collection<T> getNearbyEntitiesByType(@Nullable Class<? extends T> clazz, @NotNull Location loc, double xzRadius, double yRadius) {
        return this.getNearbyEntitiesByType(clazz, loc, xzRadius, yRadius, xzRadius, (Predicate)null);
    }

    default <T extends Entity> @NotNull Collection<T> getNearbyEntitiesByType(@Nullable Class<? extends T> clazz, @NotNull Location loc, double xRadius, double yRadius, double zRadius) {
        return this.getNearbyEntitiesByType(clazz, loc, xRadius, yRadius, zRadius, (Predicate)null);
    }

    default <T extends Entity> @NotNull Collection<T> getNearbyEntitiesByType(@Nullable Class<? extends T> clazz, @NotNull Location loc, double radius, @Nullable Predicate<? super T> predicate) {
        return this.getNearbyEntitiesByType(clazz, loc, radius, radius, radius, predicate);
    }

    default <T extends Entity> @NotNull Collection<T> getNearbyEntitiesByType(@Nullable Class<? extends T> clazz, @NotNull Location loc, double xzRadius, double yRadius, @Nullable Predicate<? super T> predicate) {
        return this.getNearbyEntitiesByType(clazz, loc, xzRadius, yRadius, xzRadius, predicate);
    }

    default <T extends Entity> @NotNull Collection<T> getNearbyEntitiesByType(@Nullable Class<? extends Entity> clazz, @NotNull Location loc, double xRadius, double yRadius, double zRadius, @Nullable Predicate<? super T> predicate) {
        if (clazz == null) {
            clazz = Entity.class;
        }

        List<T> nearby = new ArrayList();
        Iterator var11 = this.getNearbyEntities(loc, xRadius, yRadius, zRadius).iterator();

        while(true) {
            Entity vulcaniumEntity;
            do {
                do {
                    if (!var11.hasNext()) {
                        return nearby;
                    ...
                    vulcaniumEntity = (Entity)var11.next();
                } while(!clazz.isAssignableFrom(vulcaniumEntity.getClass()));
            } while(predicate != null && !predicate.test((T)vulcaniumEntity));

            nearby.add((T) vulcaniumEntity);
        }
    }
     ...
     * @param force
     */
    public void spawnParticle(@NotNull Particle particle, List<Player> receivers, Player source, double x, double y, double z, int count, double offsetX, double offsetY, double offsetZ, double extra, Object data, boolean force);
