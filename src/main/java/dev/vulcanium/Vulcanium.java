...
import dev.vulcanium.ban.BanList;
import dev.vulcanium.block.data.BlockData;
import dev.vulcanium.boss.*;
import dev.vulcanium.command.CommandException;
import dev.vulcanium.command.CommandSender;
import dev.vulcanium.command.ConsoleCommandSender;
import dev.vulcanium.command.PluginCommand;
import dev.vulcanium.entity.Entity;
import dev.vulcanium.entity.Player;
import dev.vulcanium.entity.SpawnCategory;
import dev.vulcanium.event.inventory.InventoryType;
...
import dev.vulcanium.help.HelpMap;
import dev.vulcanium.inventory.*;
import dev.vulcanium.inventory.meta.ItemMeta;
import dev.vulcanium.loot.LootTable;
import dev.vulcanium.map.MapView;
import dev.vulcanium.namespaced.Keyed;
import dev.vulcanium.namespaced.NamespacedKey;
import dev.vulcanium.packs.DataPackManager;
import dev.vulcanium.packs.ResourcePack;
import dev.vulcanium.permissions.Permissible;
import dev.vulcanium.plugin.PluginManager;
import dev.vulcanium.plugin.ServicesManager;
import dev.vulcanium.plugin.messaging.Messenger;
import dev.vulcanium.profile.PlayerProfile;
import dev.vulcanium.registry.Registry;
import dev.vulcanium.scheduler.VulcaniumScheduler;
import dev.vulcanium.scoreboard.Criteria;
import dev.vulcanium.scoreboard.ScoreboardManager;
import dev.vulcanium.structure.StructureManager;
import dev.vulcanium.tag.Tag;
import dev.vulcanium.util.CachedServerIcon;
import dev.vulcanium.world.World;
import dev.vulcanium.world.WorldBorder;
import dev.vulcanium.world.WorldCreator;
 ...
 */
public final class Vulcanium {

    private static Server server;
    private static Logger logger = Logger.getLogger("Vulcanium");
    ...
    public static void setServer(@NotNull Server server) {
        if (Vulcanium.server != null) {
            throw new UnsupportedOperationException("Cannot redefine singleton Server");
        }

        Vulcanium.server = server;
        logger.info("This server is running " + getName() + " version " + getVersion() + " (Implementing API version " + getVulcaniumVersion() + ")");
    }
     ...
     */
    @NotNull
    public static String getVulcaniumVersion() {
        return server.getVulcaniumVersion();
    }
     ...
     */
    @NotNull
    public static VulcaniumScheduler getScheduler() {
     ...
     * events.</p>
     *
     * <p>Calls {@link org.vulcanium.event.inventory.PrepareItemCraftEvent} to imitate the {@link Player}
     ...
     * <ul>
     *   <li><b>not</b> bound to a {@link org.vulcanium.entity.Boss}</li>
     *   <li>
