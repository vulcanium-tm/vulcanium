...
public abstract class PotionEffectType implements Keyed {
    ...
    private static void initializeIdMap() {
        ID_MAP = Registry.EFFECT.stream()
                .filter(type -> type instanceof PotionEffectType)
                .map(type -> (PotionEffectType) type)
                .collect(Collectors.toMap(PotionEffectType::getId, type -> type, (existing, replacement) -> existing, HashBiMap::create));
    }
    ...
    private static PotionEffectType getPotionEffectType(int typeId, @NotNull String key) {
        ...
        if (potionEffectType == null) {
            throw new IllegalStateException("No PotionEffectType found for " + namespacedKey + ". This is a bug.");
        }
