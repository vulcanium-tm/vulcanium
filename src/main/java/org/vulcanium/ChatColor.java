...
public enum ChatColor {
    ...
    public static String translateAlternateColorCodes(char altColorChar, @NotNull String textToTranslate) {
        ...
        StringBuilder result = new StringBuilder(textToTranslate);
        for (int i = 0; i < result.length() - 1; i++) {
            if (result.charAt(i) == altColorChar && "0123456789AaBbCcDdEeFfKkLlMmNnOoRrXx".indexOf(result.charAt(i + 1)) > -1) {
                result.setCharAt(i, ChatColor.COLOR_CHAR);
                result.setCharAt(i + 1, Character.toLowerCase(result.charAt(i + 1)));
            }
        }
    ...
    public static String stripColor(@Nullable final String input) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (currentChar == ChatColor.COLOR_CHAR && i < length - 1) {
                ...
                if ("0123456789AaBbCcDdEeFfKkLlMmNnOoRrXx".indexOf(nextChar) > -1) {
                    // Skip color code and the following character
                    i++;
                    continue;
                }
            }
